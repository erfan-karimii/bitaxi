...
class TestCustomerCommentView(APITestCase):
    def setUp(self) -> None:
        ...
        self.trip = baker.make(Trips,customer=customer_profile,driver=self.driver_profile)
    ...
    def test_get_customer_comments(self):
        response = self.client.get(self.url,headers={'Authorization':f'Token {self.customer_token.key}'})
        self.assertEqual(response.status_code,status.HTTP_200_OK)
        self.assertEqual(len(response.data),1)

    def test_get_notcustomer_comment(self):
        response = self.client.get(self.url,headers={'Authorization':f'Token {self.not_customer_token.key}'})
    ...
    def test_create_customer_comment(self):
        ...
        response = self.client.post(self.url,data,headers={'Authorization':f'Token {self.customer_token.key}'})

        self.assertEqual(response.status_code,201)
        self.assertEqual(Comment.objects.count(),3)
    
    def test_create_notcustomer_comment(self):
        data = {
            "customer" : self.customer.id,
            "driver" : self.driver.id,
            "trip" : self.trip.id,
        ...
        response = self.client.post(self.url,data,headers={'Authorization':f'Token {self.not_customer_token.key}'})
