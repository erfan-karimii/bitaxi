...
class TestDeleteDiscountView(APITestCase):
    ...
    def test_delete_not_superuser(self):
        ...
        response = self.client.delete(self.url,data=data,headers={'Authorization' : f'Token {self.user_token}'})
...
class TestDiscountDetailView(APITestCase):
    ...
    def test_get_discount(self):
        url = reverse("payment:discount_detail",kwargs={'code':self.valid_discount.code})
        response = self.client.get(url,headers={'Authorization' : f'Token {self.customer_token}'})
        
        self.assertEqual(response.status_code,status.HTTP_200_OK)
        self.assertDictEqual(response.data,{'code':self.valid_discount.code,'discount':self.valid_discount.discount})

    
    def test_get_not_customer(self):
        url = reverse("payment:discount_detail",kwargs={'code':self.valid_discount.code})
        response = self.client.get(url,headers={'Authorization' : f'Token {self.not_customer_token}'})
        
        self.assertEqual(response.status_code,status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data.get("detail"),"You are not a valid customer!")
    
    def test_get_passed_code(self):
        url = reverse("payment:discount_detail",kwargs={'code':self.passed_discount.code})
        response = self.client.get(url,headers={'Authorization' : f'Token {self.customer_token}'})
